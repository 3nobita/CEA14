...
app.post('/api/users/hod/bookings', async (req, res) => {
    try {
        const { driverId, DhodId, ...bookingData } = req.body;
...
app.post('/api/EMP/HOD/SRF', async (req, res) => {
    const {
        employeeCode,
        hodId,
        dateOfRequest,
    ...
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(hodId)) {
            return res.send({ message: 'Invalid HOD ID format' });
        }
        ...
        // Convert date strings to Date objects
        let dateOfRequestDate = null;
        if (dateOfRequest) {
            dateOfRequestDate = new Date(dateOfRequest);
            // Validate that the date is valid
            if (isNaN(dateOfRequestDate.getTime())) {
                return res.send({ message: 'Invalid Date Format for dateOfRequest' });
            }
        }
        ...
        res.render({ message: 'Request submitted successfully' });
    } catch (error) {
        console.error('Error:', error.message);
...
app.get('/driver/dashboard', async (req, res) => {
    // Ensure the user is authenticated and has the role of 'driver'
    if (req.session.user && req.session.user.role === 'driver') {
        const { startDate, endDate } = req.query;
        ...
        // Get the start of the next day for endDate comparison
        const tomorrow = new Date(today);
        tomorrow.setDate(today.getDate() + 1);
        
        // Default date range filter for today
        let dateFilter = {
            $gte: today,
            $lt: tomorrow
        };

        // Override dateFilter with provided query parameters if available
        if (startDate && endDate) {
        ...
        try {
            ...
            const bookings = await Request.find(query1)
        ...
        // Redirect if the user is not authenticated or is not a driver
        res.redirect('/');
    }
});
